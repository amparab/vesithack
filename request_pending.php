    ...
    <main class="pt-5 mx-lg-5">
        <div class="container-fluid mt-5">
            ...
            <div class="card mb-4 wow fadeIn">
				...
				$req_id=$_SESSION['id'];
				$query="SELECT * FROM `requests` WHERE `status`='Requested' OR `status`='Approved'";
				...
				if($query_run=mysqli_query($connect,$query))
							{
								...
								if($rows==0)
								{
									echo 'You have no asset requests pending.';
								}
								else
								{
									while($row = mysqli_fetch_assoc($query_run)) 
									{
										...
										if($status=='Requested'){
										echo'
										<form class="text-center" action="koshish.php?ID='.$reqid.'&C=0" method="post" style="width:20%;">
											<input type="submit" class="btn btn-info btn-block my-4" value="Approve"/>
										</form></div></div>
										';}
										else if($status=='Approved'){
										echo'
										<form class="text-center" action="koshish.php?ID='.$reqid.'&C=1" method="post" style="width:20%;">
											<small>Click button below if delivered to the respective stake holder</small>
											<input type="submit" class="btn btn-info btn-block my-4" value="Delivered"/>
										</form></div></div>
										';}
